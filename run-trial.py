  GNU nano 2.2.6             File: run-trial.py                                 
...
class ProtoTester(Topo):
    def __init__(self):
        ...
        receiver = self.addHost('receiver', ip='10.0.1.2', mac='00:00:00:00:00:$
...
def run_datagrump(sender, receiver):
    ...
    receiver.sendCmd('/home/ubuntu/datagrump/datagrump-receiver 9000 >/tmp/rece$
    receiver.waitOutput()
    print "done."
    print "Running datagrump-sender...",
    sender.sendCmd('/home/ubuntu/datagrump/datagrump-sender 10.0.1.2 9000 debug$
    sender.waitOutput()
    print "done."
...
def upload_data( username ):
    ...
    reply = requests.post( 'http://6829.keithw.org/cgi-bin/6829/upload-data',
                           files={'contents': (username, open( '/tmp/to-upload.$
                                                               'rb' ))} )
